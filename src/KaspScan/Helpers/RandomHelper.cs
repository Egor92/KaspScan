    ...
    public static class RandomHelper
    {
        ...
        public static T GetValue<T>(IDictionary<T, double> probabilitiesByValue)
        {
            if (probabilitiesByValue == null)
                throw new ArgumentNullException(nameof(probabilitiesByValue));
            if (probabilitiesByValue.Values.Any(x => x < 0.0))
                throw new ArgumentException("Probabilities must be not negative", nameof(probabilitiesByValue));
            var max = probabilitiesByValue.Values.Sum();
            if (!(max > 0.0))
                throw new ArgumentException("Summa of values must be positive", nameof(probabilitiesByValue));
